...
### 🖼️ GPU 全景渲染修复（v1.1.1 - 2025-12-08）
- ✅ **修复 GPU 模式全景背景不显示问题**：当前景图层透明度 > 0 时，全景背景会被覆盖为纯色
- 🔧 **根本原因**：全景层和前景层共用同一个 `uniformBuffer`，WebGPU 异步执行导致数据被覆盖
- 💡 **解决方案**：为全景渲染创建独立的 `panoUniformBuffer`，避免与前景层的 buffer 冲突
- ⚡ **新增优化**：添加 `panoSampler` 支持水平方向纹理重复（`addressModeU: 'repeat'`）

### 🎥 三维摄像机修复（v1.1.0 - 2025-12-08）
- ✅ **修复摄像机位置参数**：`cam_pos_z` 默认值从 0 改为 1000，提供合理的摄像机距离
- ✅ **改进旋转算法**：使用准确的透视投影公式，根据图层深度动态计算旋转效果
- ✅ **统一渲染行为**：GPU 和 Canvas 2D 渲染器保持一致，移除实验性标记
- ✅ **完整参数支持**：添加 `cam_pos_x`, `cam_pos_y`, `cam_pos_z` 完整支持
- 📚 **详细文档**：查看 [三维摄像机使用指南](docs/3D_CAMERA_GUIDE.md) 了解更多

### 🔧 紧急修复
- 🐛 **修复布局错乱问题**：统一类名，移除 `.ae-vue-timeline-root` 与 `.ae-timeline-root` 的嵌套冲突，解决用户安装后布局显示异常的问题
...
📚 **详细指南**：查看 [三维摄像机使用指南](docs/3D_CAMERA_GUIDE.md)
